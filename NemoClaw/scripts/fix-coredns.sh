...
# resolvectl may report IPv6; k3s-in-Docker often cannot use it from pods.
if [ -n "$UPSTREAM_DNS" ] && [[ "$UPSTREAM_DNS" == *:* ]]; then
  UPSTREAM_DNS=""
fi
...
# Secondary IPv4 resolvers: CoreDNS tries in order; helps when ISP IPv6-only
# forward targets fail from the pod network (Docker Desktop / macOS).
COREDNS_FORWARD_SERVERS="$UPSTREAM_DNS 1.1.1.1 8.8.8.8"
echo "Patching CoreDNS to forward to $COREDNS_FORWARD_SERVERS..."

docker exec "$CLUSTER" kubectl patch configmap coredns -n kube-system --type merge -p "{\"data\":{\"Corefile\":\".:53 {\\n    errors\\n    health\\n    ready\\n    kubernetes cluster.local in-addr.arpa ip6.arpa {\\n      pods insecure\\n      fallthrough in-addr.arpa ip6.arpa\\n    }\\n    hosts /etc/coredns/NodeHosts {\\n      ttl 60\\n      reload 15s\\n      fallthrough\\n    }\\n    prometheus :9153\\n    cache 30\\n    loop\\n    reload\\n    loadbalance\\n    forward . $COREDNS_FORWARD_SERVERS\\n}\\n\"}}" >/dev/null
