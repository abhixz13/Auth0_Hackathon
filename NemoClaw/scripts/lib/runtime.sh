...
# Prefer IPv4 upstream for k3s CoreDNS forward. Docker Desktop + k3s pods often
# have no working IPv6 path; using an IPv6-only ISP nameserver (e.g. 2001:558:...)
# breaks external DNS and HTTPS (integrate.api.nvidia.com, etc.).
first_ipv4_nameserver() {
  local resolv_conf="${1:-}"
...
get_colima_vm_nameserver() {
  ...
  first_ipv4_nameserver "$resolv_conf"
}
...
resolve_coredns_upstream() {
  ...
  nameserver="$(first_ipv4_nameserver "$container_resolv_conf" || true)"
  ...
  nameserver="$(first_ipv4_nameserver "$host_resolv_conf" || true)"
  if [ -n "$nameserver" ]; then
    printf '%s\n' "$nameserver"
    return 0
  fi
