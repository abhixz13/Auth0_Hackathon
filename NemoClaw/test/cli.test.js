...
describe("CLI dispatch", () => {
  ...
  it("connect does not pre-start a duplicate port forward", () => {
    ...
    const sshMarker = path.join(home, "ssh-calls");
    fs.mkdirSync(localBin, { recursive: true });
    fs.mkdirSync(registryDir, { recursive: true });
    fs.writeFileSync(
      path.join(registryDir, "sandboxes.json"),
    ...
    fs.writeFileSync(
      ...
      [
        "#!/usr/bin/env bash",
        `marker_file=${JSON.stringify(sshMarker)}`,
        "printf '%s\\n' \"$*\" >> \"$marker_file\"",
        "exit 0",
      ].join("\n"),
      { mode: 0o755 }
    ...
    expect(calls).toContain("sandbox ssh-config alpha");
    expect(calls.some((call) => call.startsWith("forward start --background 18789"))).toBe(false);
    const sshLine = fs.readFileSync(sshMarker, "utf8").trim();
    expect(sshLine).toContain("ServerAliveInterval=30");
    expect(sshLine).toContain("openshell-alpha");
  });
