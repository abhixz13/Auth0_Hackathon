...
function parseSshConfigHostAlias(sshConfigText) {
  const match = sshConfigText.match(/^Host\s+(\S+)/m);
...
async function sandboxConnect(sandboxName) {
  ...
  if (process.env.NEMOCLAW_OPENSHELL_CONNECT === "1") {
    const legacy = spawnSync(getOpenshellBinary(), ["sandbox", "connect", sandboxName], {
      stdio: "inherit",
      cwd: ROOT,
      env: process.env,
    });
    exitWithSpawnResult(legacy);
    return;
  }

  let sshConfigText;
  try {
    sshConfigText = execFileSync(getOpenshellBinary(), ["sandbox", "ssh-config", sandboxName], {
      encoding: "utf-8",
      cwd: ROOT,
      env: process.env,
    });
  } catch {
    const fallback = spawnSync(getOpenshellBinary(), ["sandbox", "connect", sandboxName], {
      stdio: "inherit",
      cwd: ROOT,
      env: process.env,
    });
    exitWithSpawnResult(fallback);
    return;
  }

  const hostAlias = parseSshConfigHostAlias(sshConfigText);
  if (!hostAlias) {
    const fallback = spawnSync(getOpenshellBinary(), ["sandbox", "connect", sandboxName], {
      stdio: "inherit",
      cwd: ROOT,
  ...
  const cfgPath = path.join(tmpDir, "config");
  try {
    fs.writeFileSync(cfgPath, sshConfigText, { mode: 0o600 });
    const sshArgs = [
      "-F", cfgPath,
    ...
    if (result.error && result.error.code === "ENOENT") {
      const fb = spawnSync(getOpenshellBinary(), ["sandbox", "connect", sandboxName], {
        stdio: "inherit",
        cwd: ROOT,
        env: process.env,
      });
      exitWithSpawnResult(fb);
      return;
    }
    exitWithSpawnResult(result);
  } finally {
